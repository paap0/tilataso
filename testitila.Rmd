...
df1<-df1[-2]
df1a<-df1
df1a$EMKUOL<-tilat$EMKUOLLJAKO
head(df1a)

CreateTableOne(vars=keeps1,strata=c("EMKUOL"),factorVars=kat1,data=df1a )
...
df2a<-df2
df2a$EMKUOL<-tilat$EMKUOLLJAKO


CreateTableOne(vars=keeps2,strata=c("EMKUOL"),factorVars=kat2,data=df2a)
...
ja
```{r fig.align="center", fig.height=10, fig.width=10, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
df3a<-df3
df3a$EMKUOL<-tilat$EMKUOLLJAKO



CreateTableOne(vars=keeps3,strata=c("EMKUOL"),factorVars=kat3,data=df3a)
...
#Tama on kategoristen setti
kuvat41<- kuvat4 %>% dplyr::select(one_of(keeps4))%>%  dplyr::select(-one_of(num4))

#ja kategoriset muuttujat
kat4<-colnames(kuvat41)
...
```{r echo=TRUE, fig.align="center", fig.width=10, message=FALSE, warning=FALSE, warnings=FALSE}
kuvat4$Muutelkaynn_0ei_1_satunn_2kaynnmuusaann<-ifelse(kuvat4$Muutelkaynn_0ei_1_satunn_2kaynnmuusaann>0,1,0)

df4<- dplyr::select(kuvat4, one_of(keeps4))
df4a<-df4
df4a$EMKUOL<-tilat$EMKUOLLJAKO


CreateTableOne(vars=keeps4,strata=c("EMKUOL"),factorVars=kat4,data=df4a)

```

Tässä kohtaa vähän MCA-testailua, mutta muuttujia on valtavasti liikaa.

```{r echo=TRUE, fig.align="center", message=FALSE, warning=FALSE, warnings=FALSE}
dfall<-cbind(df1,df2,df3,df4)
dfall$EMKUOL<-tilat$EMKUOLLJAKO
dfall$Emakotcat<-ifelse(dfall$Emakot>400,1,0)
dfall$Ensikotcat<-ifelse(dfall$Ensikot>0,1,0)
dfall$Lihascat<-ifelse(dfall$Lihasiat_tontilla>0,1,0)
dfall$Porsab<-ifelse(dfall$Pors_ABper100>10,1,0)
dfall$PorsNSAIDS<-ifelse(dfall$Pors_NSAIDper100>12,1,0)
summary(as.numeric(dfall$Tiineet_ABper101))
summary(as.numeric(dfall$Tiineet_NSAIDper100))

dfall$TIINSAIDS<-ifelse(dfall$Tiineet_NSAIDper100>4,1,0)
dfall$TIIAB<-ifelse(dfall$Tiineet_ABper101>4,1,0)
summary(as.numeric(dfall$Pors_NSAIDper100))
dfall<-dfall[ , !names(dfall) %in% c("Emakot","Ensikot","Karjut_haju","Lihasiat_tontilla","Pors_ABper100","Pors_NSAIDper100","Tiineet_ABper101","Tiineet_NSAIDper100")] 
dfall<-dfall %>% mutate_all(as.factor)
tea_mca <- MCA(dfall, graph = FALSE)
summary(tea_mca)
```
...
```
```{r eval=FALSE, fig.align="center", fig.height=8, fig.width=8, message=FALSE, warning=FALSE, include=FALSE}
 # MCA plot of observations and categories
cats = apply(dfall, 2, function(x) nlevels(as.factor(x)))
...
ggplot(data = mca1_vars_df, aes(x = Dim.1, y = Dim.2)) +
  ...
  ggtitle("MCA plot of variables using part of the data") +
